...
// ✅ Session setup
app.use(session({
  secret: 'dogwalksecret', // Use a secure, random string in production
  resave: false,
...
app.post('/login', async (req, res) => {
  ...
  try {
    ...
    if (rows.length === 0) {
      return res.json({ success: false, message: 'Invalid or incorrect credentials' });
    }
    ...
    if (user.role === 'owner') {
      ...
      return res.json({ success: false, message: 'Unknown user role' });
    }
...
// ✅ Export the app (used by index.js or start.js)
module.exports = app;
